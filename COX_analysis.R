...
plot(data.KM1, conf.int=F, xlab="Time (days)", ylab="Survival = S(t)", main="KM-Model", col=c("red","blue", "green"), las=1, lwd=2, mark.time=TRUE)
...
cox.num1 <- coxph( Surv(data1$Time, data1$Status) ~ data1$P5_cat)
summary(cox.num1)
...
cox.num4 <- coxph( Surv(data1$Time, data1$Status) ~ data1$Vol_P5 + data1$MRI_T3 + data1$Margins)
summary(cox.num4)
