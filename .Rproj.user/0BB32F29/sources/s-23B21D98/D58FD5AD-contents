...
######################################################
  ...
  inData <- read.csv(g, header=TRUE, stringsAsFactors=FALSE)
  inData$Censor = inData$Status
  inData$Variable1 = inData$max3Ddiameter_cat
  ...
  surv.object <- Surv(inData$Time, inData$Status)
  myfits.v4 <- survfit(surv.object~Variable1, data = inData)
  myfits.v4.LR = survdiff(surv.object~Variable1,data=inData)
  ...
  v4_cph = coxph(surv.object ~ Variable1,data=inData)
  summary(v4_cph)
...
######################################################
 ...
 #  x_cox <- inData[, names(inData) %in% c("Time","Outcome","Variable1","Variable2","Variable3")]
  ...
  x_cox <- inData[, names(inData) %in% c("Time","Status","x", "minusSVR", "Path_EPE", "Path_SVI")]
