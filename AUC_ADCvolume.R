...
# pred appears to contain the scores
head(pred)
head(data1)
...
auc <- round(auc, 4)
legend(.65, .2, auc, title = "AUC", cex = 0.8)

pr <- evalm

library(OptimalCutpoints)

optimal.cutpoint.SpEqualSe <- optimal.cutpoints(X = "PIRADS5", status = "Status", tag.healthy = 0, 
                                                methods = "SpEqualSe", data = data1, pop.prev = NULL, categorical.cov = "PIRADS5", 
